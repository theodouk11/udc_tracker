...
@MODELS.register_module()
class CustomDetTTAModel(BaseTTAModel):
    """Custom TTA model for detection, supporting crop and resize transformations.

    Args:
        tta_cfg (dict, optional): Configuration for TTA, including NMS settings.
    """
    def __init__(self, tta_cfg=None, **kwargs):
        super().__init__(**kwargs)
        self.tta_cfg = tta_cfg

    def merge_aug_bboxes(self, aug_bboxes: List[Tensor], aug_scores: List[Tensor],
                         img_metas: List[dict]) -> Tuple[Tensor, Tensor]:
        """Merge augmented detection bboxes and scores, handling crop and resize.

        Args:
            aug_bboxes (List[Tensor]): List of bboxes with shape (n, 4*#class).
            aug_scores (List[Tensor]): List of scores with shape (n, #class).
            img_metas (List[dict]): List of image metainfo, including
                'ori_shape', 'flip', 'flip_direction', 'crop_offset', 'resize_scale'.

        Returns:
            Tuple[Tensor, Tensor]: Merged bboxes (n, 4) and scores (n,).
        """
        recovered_bboxes = []
        for bboxes, img_info in zip(aug_bboxes, img_metas):
            ori_shape = img_info['ori_shape']
            flip = img_info.get('flip', False)
            flip_direction = img_info.get('flip_direction', None)
            crop_offset = img_info.get('crop_offset', (0, 0))  # (x1, y1)
            resize_scale = img_info.get('resize_scale', (1.0, 1.0))  # (scale_w, scale_h)

            # Step 1: Undo resize (scale bboxes back to crop size)
            if resize_scale != (1.0, 1.0):
                scale_w, scale_h = resize_scale
                bboxes[:, [0, 2]] /= scale_w  # Undo x scaling
                bboxes[:, [1, 3]] /= scale_h  # Undo y scaling

            # Step 2: Undo crop (add crop offset)
            if crop_offset != (0, 0):
                x_offset, y_offset = crop_offset
                bboxes[:, [0, 2]] += x_offset  # Add x offset
                bboxes[:, [1, 3]] += y_offset  # Add y offset

            # Step 3: Undo flip if applied
            ...
            # Clip bboxes to original image boundaries
            bboxes[:, [0, 2]] = bboxes[:, [0, 2]].clamp(min=0, max=ori_shape[1])
            bboxes[:, [1, 3]] = bboxes[:, [1, 3]].clamp(min=0, max=ori_shape[0])
    ...
    def _merge_single_sample(self, data_samples: List[DetDataSample]) -> DetDataSample:
        """Merge predictions from different views of one image.

        Args:
            data_samples (List[DetDataSample]): Predictions of enhanced data for one image.
        ...
        assert data_samples[0].pred_instances.get('masks', None) is None, \
            'TTA of instance segmentation is not supported.'
