...
class TopkHungarianAssigner(TaskAlignedAssigner):
    ...
    def assign(self,
        ...
        Args:
            pred_instances (:obj:`InstanceData`, optional): Instances of model
                ...
                in other places.
            gt_instances (:obj:`InstanceData`, optional): Ground truth of instance
                annotations. It usually includes ``bboxes``, with shape (k, 4),
                ``labels``, with shape (k, ).
            img_meta (dict, optional): Meta information for one image.
            pred_scores (Tensor, optional): Predicted normalized classification
                scores for one image, has shape (num_dense_queries,
                cls_out_channels). Used for backward compatibility.
            decode_bboxes (Tensor, optional): Predicted unnormalized bbox coordinates
                for one image, has shape (num_dense_queries, 4) with the
                last dimension arranged as (x1, y1, x2, y2). Used for backward compatibility.
            gt_bboxes (Tensor, optional): Unnormalized ground truth
                bboxes for one image, has shape (num_gt, 4) with the
                last dimension arranged as (x1, y1, x2, y2). Used for backward compatibility.
                NOTE: num_gt is dynamic for each image.
            gt_labels (Tensor, optional): Ground truth classification
                    index for the image, has shape (num_gt,). Used for backward compatibility.
                    NOTE: num_gt is dynamic for each image.
        ...
        """
        # 兼容两种调用方式：
        # 1. 新方式：pred_instances, gt_instances, img_meta (兼容HungarianAssigner)
        # 2. 旧方式：pred_scores, decode_bboxes, gt_bboxes, gt_labels, img_meta
        if pred_instances is not None and gt_instances is not None:
            # 新方式：从InstanceData中提取参数
            pred_scores = pred_instances.scores
            decode_bboxes = pred_instances.bboxes
            gt_bboxes = gt_instances.bboxes
            gt_labels = gt_instances.labels
        elif pred_scores is not None and decode_bboxes is not None and gt_bboxes is not None and gt_labels is not None:
            # 旧方式：直接使用传入的参数
            pass
        else:
            raise ValueError("Either (pred_instances, gt_instances) or (pred_scores, decode_bboxes, gt_bboxes, gt_labels) must be provided")
        ...
        # 如果pred_instances和gt_instances不是从参数传入的，则创建它们
