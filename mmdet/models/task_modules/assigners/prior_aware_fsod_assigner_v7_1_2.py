...
class FSODAssigner_v7_1_2(BaseAssigner):
    """FSOD Assigner v7.1.2 with Semantic-Spatial Consistency Check.
    ...
    5. NEW: Semantic-spatial consistency check combining IoU and feature similarity
    ...
    The assignment process includes:
     ...
     3. Semantic-spatial consistency filtering using both IoU and feature similarity
    ...
    def _semantic_spatial_consistency_filter(self, 
                                           ...
                                           gt_bboxes: Tensor,
                                           gt_features: Optional[Tensor],
                                           matched_pred_indices: Tensor) -> Tensor:
        """语义-空间一致性过滤。
        
        针对未标注正样本检测，寻找IoU较小但特征相似性较高的预测：
        ...
        Args:
            ...
            gt_features (Tensor, optional): GT特征
        ...
        if (not self.enable_semantic_spatial_check or 
            pred_features is None or gt_features is None):
            return unmatched_indices
        ...
        feature_sim = self._compute_feature_similarity(unmatched_features, gt_features)  # [num_unmatched, num_gt]
        ...
        consistency_scores = feature_sim - alpha * ious
        
        # 对每个未匹配预测，找到最高一致性分数
        max_consistency_scores, _ = consistency_scores.max(dim=1)
        
        # 过滤：保留一致性分数高于阈值的预测
        # 这意味着特征相似性高且IoU相对较小的预测会被保留
        consistency_mask = max_consistency_scores >= self.feature_similarity_threshold
    ...
    def _detect_unlabeled_positives(
        ...
        if self.enable_semantic_spatial_check and len(final_candidate_indices) > 0:
            ...
            gt_features = getattr(gt_instances, 'features', None)
