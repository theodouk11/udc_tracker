...
class FSODAssigner_v7_3(BaseAssigner):
    """FSOD Assigner v7.3 - 添加不确定性感知分配功能
    
    基于v7.2版本，新增功能：
    1. 继承v7.1的动态置信度校准
    2. 继承v7.2的语义-空间一致性检查
    3. 不确定性估计：基于预测分数方差和熵
    4. 四类标签系统：正样本(1)、背景(0)、未标注正样本(-1)、不确定样本(-2)
    5. 不确定性感知过滤机制
    
    验证目标：证明不确定性感知分配能够更好地处理模糊样本
    """
    ...
    def __init__(
        ...
        uncertainty_method: str = 'entropy',  # 'entropy', 'variance', 'combined'
    ...
    def _estimate_uncertainty(self, pred_instances: InstanceData) -> Tensor:
        ...
        Returns:
            Tensor: 不确定性分数，形状为 (num_preds,)，值越高表示越不确定
        """
    ...
    def _detect_unlabeled_positives(
        ...
        if not self.enable_unlabeled_detection:
            empty_tensor = torch.empty(0, dtype=torch.long, device=assigned_gt_inds.device)
        ...
        # 应用空间NMS到未标注正样本
        if len(unlabeled_positive_indices) > 0:
            final_bboxes = pred_bboxes[unlabeled_positive_indices]
            final_scores = pred_scores[unlabeled_positive_indices]
            
            final_labels = None
            if hasattr(pred_instances, 'scores') and pred_instances.scores is not None:
                candidate_scores = pred_instances.scores[unlabeled_positive_indices]
                if candidate_scores.dim() > 1 and candidate_scores.size(-1) > 1:
                    final_labels = torch.argmax(candidate_scores, dim=-1)
                elif hasattr(pred_instances, 'labels') and pred_instances.labels is not None:
                    final_labels = pred_instances.labels[unlabeled_positive_indices]
            
            if self.use_nms:
                keep_indices = self._spatial_nms(
                    final_bboxes, final_scores, self.spatial_nms_threshold, final_labels
                )
                unlabeled_positive_indices = unlabeled_positive_indices[keep_indices]
        
        # 限制未标注正样本数量
        if len(unlabeled_positive_indices) > self.max_unlabeled_positives:
            final_scores = pred_scores[unlabeled_positive_indices]
            sorted_indices = torch.argsort(final_scores, descending=True)
            unlabeled_positive_indices = unlabeled_positive_indices[sorted_indices[:self.max_unlabeled_positives]]
    ...
    def assign(
        ...
        **kwargs
        ...
        # 检测未标注正样本和不确定样本
        unlabeled_indices, uncertain_indices = self._detect_unlabeled_positives(
    ...
    def get_uncertain_samples(
        ...
        """获取不确定样本"""
        assign_result = self.assign(pred_instances, gt_instances, img_meta)
        uncertain_mask = assign_result.labels == self.uncertain_label_value
    ...
    def get_calibration_stats(self) -> dict:
        ...
        return {
            ...
            'uncertainty_estimation_enabled': self.enable_uncertainty_estimation,
