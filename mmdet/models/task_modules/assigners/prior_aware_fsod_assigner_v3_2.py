...
class PriorAwareFSODAssigner_v3_2(BaseAssigner):
    """Prior-Aware Assigner for Few-Shot Object Detection with Entropy-based Uncertainty.
    ...
    2. Identify potential unlabeled positives based on confidence, entropy, and IoU constraints
    3. Use a three-class labeling system: 1 (matched queries), 0 (background), -1 (potential unlabeled positive)
    4. Assign potential unlabeled positives based on: unmatched + high confidence + low entropy + low IoU with GT
    ...
    The assignment process includes:
     ...
     2. Detection of potential unlabeled positives: unmatched queries with confidence higher than
        max confidence of matched queries AND entropy lower than min entropy of matched queries
        AND IoU with GT lower than min IoU of matched queries
     3. All other unmatched queries are assigned as background (label 0)
    
    The entropy-based uncertainty helps identify queries that are not only confident but also
    have low uncertainty in their predictions, making them more likely to be true positives.
    ...
    Args:
        ...
        unlabeled_label_value (int): Label value for potential unlabeled positives.
            These are unmatched queries with high confidence, low entropy but low IoU with GT. Default: -1.
    ...
    def _compute_entropy(self, scores: Tensor) -> Tensor:
        """Compute entropy for classification scores.
        
        Args:
            scores (Tensor): Classification scores, shape (N, num_classes) or (N,).
            
        Returns:
            Tensor: Entropy values, shape (N,).
        """
        if scores.dim() == 1:
            # For single class scores, entropy is 0 (no uncertainty)
            return torch.zeros(scores.size(0), device=scores.device, dtype=scores.dtype)
        else:
            # Handle edge case where there's only one class (max_text_len=1)
            if scores.size(-1) == 1:
                # If only one class, entropy is 0 (no uncertainty)
                return torch.zeros(scores.size(0), device=scores.device, dtype=scores.dtype)
            else:
                # For multi-class scores, apply softmax to get probabilities
    ...
    def _detect_unlabeled_positives(
        ...
        1. High confidence (higher than max confidence of matched queries)
        2. Low entropy (lower than min entropy of matched queries)
        3. Low IoU with GT (lower than min IoU of matched queries)
        ...
        # Store original scores for entropy calculation
        original_scores = pred_scores
        
        # Handle different score formats for confidence
        ...
        # Compute entropy for all predictions
        pred_entropy = self._compute_entropy(original_scores)  # (num_queries,)
        ...
        matched_entropy = pred_entropy[matched_indices]
        matched_bboxes = pred_bboxes[matched_indices]
        
        # Get maximum confidence and minimum entropy of matched queries
        max_matched_confidence = torch.max(matched_scores).item()
        min_matched_entropy = torch.min(matched_entropy).item()
        ...
        # Compute IoU for each matched query with its specific assigned GT
        # Use batch computation for efficiency
        matched_ious_list = []
        for i, gt_idx in enumerate(matched_gt_indices):
            iou = self._compute_bbox_iou(
                matched_bboxes[i:i+1], 
                gt_bboxes[gt_idx:gt_idx+1]
            )[0, 0]
            matched_ious_list.append(iou)
        
        matched_ious_with_assigned_gt = torch.stack(matched_ious_list)
        min_matched_iou = torch.min(matched_ious_with_assigned_gt).item()
        ...
        unassigned_entropy = pred_entropy[unassigned_indices]
        
        # Filter by confidence: higher than max confidence of matched queries
        high_conf_mask = unassigned_scores >= max_matched_confidence
        if not high_conf_mask.any():
            return torch.empty(0, dtype=torch.long, device=assigned_gt_inds.device)
        
        # Filter by entropy: lower than min entropy of matched queries
        low_entropy_mask = unassigned_entropy <= min_matched_entropy
        
        # Combine confidence and entropy filters
        combined_mask = high_conf_mask & low_entropy_mask
        if not combined_mask.any():
            return torch.empty(0, dtype=torch.long, device=assigned_gt_inds.device)
        
        candidate_indices = unassigned_indices[combined_mask]
        candidate_bboxes = unassigned_bboxes[combined_mask]
        
        # Compute IoU between candidates and all GT boxes
        ious = self._compute_bbox_iou(candidate_bboxes, gt_bboxes)  # (N_candidates, N_gt)
        
        # Handle edge case where there's only one GT
        if ious.size(1) == 1:
            max_ious_with_gt = ious.squeeze(1)  # (N_candidates,)
            # Ensure it's still a 1D tensor even with single candidate
            if max_ious_with_gt.dim() == 0:
                max_ious_with_gt = max_ious_with_gt.unsqueeze(0)
        else:
    ...
    def assign(
        ...
        **kwargs
        ...
        2. Detection of potential unlabeled positives: high confidence + low entropy
        ...
        # These are unmatched queries with high confidence and low entropy
