...
@weighted_loss
def marginal_iou_loss(pred: Tensor,
                      target: Tensor,
                      margin: float = 0.5,
                      eps: float = 1e-6) -> Tensor:
    """Marginal IoU loss.

    Computing the marginal IoU loss between a set of predicted bboxes and target bboxes.
    The loss is calculated as 1 - IoU when IoU > margin, 0 otherwise.
    This loss encourages the IoU to be below the margin threshold.

    Args:
        pred (Tensor): Predicted bboxes of format (x1, y1, x2, y2),
            shape (n, 4).
        target (Tensor): Corresponding gt bboxes, shape (n, 4).
        margin (float): Margin threshold for IoU. Default: 0.5.
        eps (float): Epsilon to avoid log(0).
    ...
    # Use 1 - IoU when IoU > margin, 0 otherwise
    mask = ious > margin
    loss = torch.zeros_like(ious)
    loss[mask] = 1 - ious[mask]
    
    return loss


@MODELS.register_module()
class MarginalIoULoss(nn.Module):
    """Marginal IoU Loss.

    Computing the marginal IoU loss between a set of predicted bboxes and target bboxes.
    The loss is calculated as -max(0, IoU(a, b) - margin).log().
    This loss encourages the IoU to be below the margin threshold.
    ...
    def forward(self,
        ...
        if weight is not None and weight.dim() > 1:
            # TODO: remove this in the future
            # reduce the weight of shape (n, 4) to (n,) to match the
            # marginal_iou_loss of shape (n,)
            assert weight.shape == pred.shape
            weight = weight.mean(-1)
        loss = self.loss_weight * marginal_iou_loss(
            pred,
            target,
            weight,
            margin=self.margin,
