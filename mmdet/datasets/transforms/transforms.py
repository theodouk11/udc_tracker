...
@TRANSFORMS.register_module()
class HumanityRandomCrop(RandomCrop):
    """Random crop the image & bboxes & masks with bbox size preservation.

    This class extends RandomCrop to ensure that each bbox remaining in the image
    after cropping retains at least 70% of its original size. If this condition
    is not met, the cropping is retried.

    Args:
        min_bbox_size_ratio (float): Minimum ratio of bbox size to preserve.
            Defaults to 0.7 (70%).
        max_retry_attempts (int): Maximum number of retry attempts for cropping.
            Defaults to 10.
        **kwargs: Other arguments passed to RandomCrop.
    """
    ...
    def __init__(self,
                 ...
                 min_bbox_size_ratio: float = 0.7,
                 max_retry_attempts: int = 5) -> None:
        super().__init__(crop_size, crop_type, allow_negative_crop,
                         recompute_bbox, bbox_clip_border)
        self.min_bbox_size_ratio = min_bbox_size_ratio
        self.max_retry_attempts = max_retry_attempts

    def _check_bbox_size_preservation(self, original_bboxes, cropped_bboxes,
                                      valid_inds) -> bool:
        """Check if cropped bboxes preserve minimum size ratio.

        Args:
            original_bboxes: Original bboxes before cropping.
            cropped_bboxes: Bboxes after cropping and clipping.
            valid_inds: Indices of valid bboxes after cropping.

        Returns:
            bool: True if all valid bboxes preserve minimum size ratio.
        """
        if not valid_inds.any():
            return True

        # Get original and cropped areas for valid bboxes
        original_valid_bboxes = original_bboxes[valid_inds]
        original_areas = original_valid_bboxes.areas
        cropped_areas = cropped_bboxes.areas

        # Calculate size preservation ratios
        size_ratios = cropped_areas / (original_areas + 1e-8)  # Add small epsilon to avoid division by zero

        # Check if all ratios meet the minimum requirement
        return (size_ratios >= self.min_bbox_size_ratio).all()

    def _crop_data(self, results: dict, crop_size: Tuple[int, int],
                   allow_negative_crop: bool) -> Union[dict, None]:
        """Function to randomly crop images with bbox size preservation check.
        ...
        # Store original bboxes for size comparison
        original_bboxes = None
        if results.get('gt_bboxes', None) is not None:
            original_bboxes = results['gt_bboxes'].clone()

        for attempt in range(self.max_retry_attempts):
            # Reset results for each attempt
            ...
            homography_matrix = np.array(
                [[1, 0, -offset_w], [0, 1, -offset_h], [0, 0, 1]],
                dtype=np.float32)
            if current_results.get('homography_matrix', None) is None:
                current_results['homography_matrix'] = homography_matrix
            else:
                current_results['homography_matrix'] = homography_matrix @ current_results[
                    'homography_matrix']

            # crop the image
            cropped_img = img[crop_y1:crop_y2, crop_x1:crop_x2, ...]
            img_shape = cropped_img.shape
            current_results['img'] = cropped_img
            current_results['img_shape'] = img_shape[:2]

            # crop bboxes accordingly and clip to the image boundary
            if current_results.get('gt_bboxes', None) is not None:
                bboxes = current_results['gt_bboxes'].clone()
                bboxes.translate_([-offset_w, -offset_h])
                if self.bbox_clip_border:
                    bboxes.clip_(img_shape[:2])
                valid_inds = bboxes.is_inside(img_shape[:2]).numpy()
                ...
                # Check bbox size preservation
                if original_bboxes is not None and valid_inds.any():
                    if not self._check_bbox_size_preservation(original_bboxes, bboxes[valid_inds], valid_inds):
                        if attempt == self.max_retry_attempts - 1:
                            # If max attempts reached, use the last attempt result
                            break
                        continue

                current_results['gt_bboxes'] = bboxes[valid_inds]

                if current_results.get('gt_ignore_flags', None) is not None:
                    current_results['gt_ignore_flags'] = \
                        current_results['gt_ignore_flags'][valid_inds]

                if current_results.get('gt_bboxes_labels', None) is not None:
                    current_results['gt_bboxes_labels'] = \
                        current_results['gt_bboxes_labels'][valid_inds]

                if current_results.get('gt_masks', None) is not None:
                    current_results['gt_masks'] = current_results['gt_masks'][
                        valid_inds.nonzero()[0]].crop(
                            np.asarray([crop_x1, crop_y1, crop_x2, crop_y2]))
                    if self.recompute_bbox:
                        current_results['gt_bboxes'] = current_results['gt_masks'].get_bboxes(
                            type(current_results['gt_bboxes']))

                # We should remove the instance ids corresponding to invalid boxes.
                if current_results.get('gt_instances_ids', None) is not None:
                    current_results['gt_instances_ids'] = \
                        current_results['gt_instances_ids'][valid_inds]

            # crop semantic seg
            if current_results.get('gt_seg_map', None) is not None:
                current_results['gt_seg_map'] = current_results['gt_seg_map'][crop_y1:crop_y2,
                                                              crop_x1:crop_x2]

            # If we reach here, the crop is successful
            return current_results

        # If all attempts failed, return the original image to preserve all bboxes
        return results
    ...
    def __repr__(self) -> str:
        ...
        repr_str += f'recompute_bbox={self.recompute_bbox}, '
        repr_str += f'bbox_clip_border={self.bbox_clip_border}, '
        repr_str += f'min_bbox_size_ratio={self.min_bbox_size_ratio}, '
        repr_str += f'max_retry_attempts={self.max_retry_attempts})'
        return repr_str
